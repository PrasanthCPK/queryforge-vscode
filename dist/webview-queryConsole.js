...
function getOrCreateTargetTab(label) {
...
function renderTable(tab) {
  ...
  `;
    ...
    resultsContent.querySelectorAll('thead th').forEach(th => {
...
function requestPage(tab, page) {
    if (!tab.sql || !tab.connectionId)
        return;
    setStatus('Loading…');
...
function renderExplain(tab) {
    ...
    resultsContent.querySelectorAll('.node-header').forEach(hdr => {
...
window.addEventListener('message', (event) => {
    ...
    switch (msg.type) {
        ...
        case 'selectConnection':
            if (msg.connectionId)
                connSelect.value = msg.connectionId;
            break;
        case 'loadScript':
...
function populateConnections(list) {
...
/******/ })()
;
//# sourceMappingURL=webview-queryConsole.js.map
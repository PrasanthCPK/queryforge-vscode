...
const explainPanel = document.getElementById('tab-explain');
...
function renderTable(result) {
    ...
    const body = sorted.map(row => `<tr>${result.columns.map(col => {
        const v = row[col];
...
function renderExplain(node) {
    ...
    explainPanel.querySelectorAll('.node-header').forEach(hdr => {
